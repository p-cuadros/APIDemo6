...
app.MapGet("/AuthorizedResource", (Func<string>)([Authorize] () => "Action Succeeded")).Produces(StatusCodes.Status200OK)
...
app.MapGet("/books_by_page", async (int pageNumber,int pageSize, BooksDB db) =>
    await db.Books.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync()
...
app.MapPost("/books",[AllowAnonymous] async ([FromBody] Book addbook,[FromServices] BooksDB db, HttpResponse response) =>
    {
...
app.MapPut("/books",[AllowAnonymous] async (int bookID,string bookTitle, [FromServices] BooksDB db, HttpResponse response) =>
    {
